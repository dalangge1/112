...
%for file = file_names'
    ...
    [tB, tG, tR] = pyramidalign(B, G, R);
    pyr = cat(3, R, G, B);
    imwrite(pyr, [output_dir file_name(1:end - length(file_ext) - 1) '_pyr.jpg'])
...
%     imshow(new);
    ...
    imwrite(orig, [output_dir file_name(1:end - length(file_ext) - 1) '_orig.jpg'])
    imwrite(new, [output_dir file_name(1:end - length(file_ext) - 1) '_xc.jpg'])
